...
# Writer
writer_llm = ChatGroq(model="openai/gpt-oss-120b", temperature=0.2)
...
def write_node(state: State)->dict:
  ...
  return {
    "messages" : [("human",user_message),response], # AI will directly know the "response" is the AI's response to the user message, so we can store it as a message
    "attempt" : attempt
  }
...
def reviewer_node(state: State)->dict:
  ...
  is_approved = "APPROVED" in review_text.upper().split("FEEDBACK")[0] # splitting the list on basis of feedback and using the firsr part at 0th index to check if it contains approved or not as you can see in the REVIEWER_SYSTEM_PROMPT we have mentioned that the first line should contain the verdict and the second line should contain the feedback so we are splitting the response on basis of feedback and checking if the first part contains approved or not
...
def should_stop_looping(state: State):
  if state["is_approved"]:
    print("Post approved! Stopping loop.")
    return END
  if state["attempt"] >= 3:
