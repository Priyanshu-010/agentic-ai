...
def merge_score_dicts(existing: dict, newUpdated:dict) -> dict:
  if existing is None:
    return newUpdated
  return {**existing, **newUpdated}

# So basically this is like a spread operator in JS but it updates the existing dict with the new one. Read below for more

# ** inside a dict literal is dictionary unpacking — it "spreads" all the key-value pairs of a dict into the new dict being built

# Key behavior: if a key exists in both dicts, the one on the right wins (it overwrites).  So in your function, newUpdated takes priority over existing for any shared keys
...
# {**existing, **newUpdated}
# → {"a": 1, "b": 99, "c": 3}
#         ^^^ b=99 from newUpdated overwrote b=2   
